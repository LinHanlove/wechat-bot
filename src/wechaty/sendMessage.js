...
// 从环境变量中导入联系人白名单
const aliasWhiteList = env.ALIAS_WHITELIST
  ? env.ALIAS_WHITELIST.split(",")
  : [];
...
export async function defaultMessage(msg, bot, ServiceType = "GPT") {
  ...
  try {
    ...
    if (isRoom && room) {
      ...
      const question =
        (await msg.mentionText()) || content.replace(`${botName}`, ""); // 去掉艾特的消息主体
    ...
    if (!isRoom && room) {
      ...
      if (typeof response === "string") {
        ...
        // 发送视频
        const videoBuffer = await downloadVideo(response.video);
        if (videoBuffer) {
